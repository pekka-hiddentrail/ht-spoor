...
class Transaction:
    """One timed browser round-trip: its operation, duration, and a context label.
...
class _TimingDriver:
    ...
    def _timed(self, op: str, func: Callable[..., Any], *args: Any) -> Any:
        ...
        try:
            ...
            self.transactions.append(Transaction(op, duration, _detail(args)))
...
def measure(
    ...
    reasons. `transactions` are the `_TimingDriver` records, reduced here both to
    per-operation `OpStat`s and to the slowest-transaction shortlist.
    """
...
def benchmark_entries(metrics: BenchMetrics, label: str) -> list[dict[str, object]]:
    ...
    counters are gated by `check_drift`, never trended. Every row is namespaced by
    `label` so several sets share one dashboard without colliding.
    """
