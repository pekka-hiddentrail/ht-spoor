...
**Turning a map into regression tests.** Add `--gen-tests <dir>` to also write a runnable
**pytest suite** of the map to that directory — one test per mapped transition that
replays the path to that screen, fires the action, and asserts the same things changed
(the console lines, storage keys, and network requests Spoor recorded). Re-run the suite
against the live site later and a failing test means the site's behaviour drifted from the
map. Running the suite drives a real browser, so it needs Playwright installed; any
secrets captured along the way are redacted before they reach a test.

```bash
spoor explore https://example.com --gen-tests ./tests-from-map
