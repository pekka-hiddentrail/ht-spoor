...
- Exploration mode: point Spoor at a URL with no config and it maps the site in a real browser — `spoor explore <url>` presses the buttons and follows the links on each screen, recording where each one leads as a graph of states and transitions (see "Exploring a site" below for the safety rule). Add `--wiki <dir>` to render the result as a browsable wiki — an overview diagram plus a page per state and transition, secrets redacted. Each exploration is also remembered in the local map, so the serving layer below can hand the graph back later without re-exploring
