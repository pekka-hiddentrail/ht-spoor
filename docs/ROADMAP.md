...
- **Deferred to later §2b slices:** GraphQL introspection (layer 2), passive-capture spec synthesis (layers 3–4, gated on the mitmproxy2swagger spike), and action-to-endpoint correlation (layer 5). Layer 1 itself is now complete — both conventional-path probing and reference-scanning (landing HTML *and* same-origin JS bundles) ship (see the next note). Also deferred: sharing one robots cache / politeness gate between the resolver and discovery (today each builds its own, so a run fetches `robots.txt` twice) — a bounded, known optimization, not a correctness gap.
