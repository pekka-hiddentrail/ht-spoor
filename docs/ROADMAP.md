...
- **Deferred to later §2b slices:** scanning referenced JS bundles for a spec reference (the *landing-page HTML* half of layer 1 now ships — see the next note; scanning each `<script src>` bundle is the remaining part), GraphQL introspection (layer 2), passive-capture spec synthesis (layers 3–4, gated on the mitmproxy2swagger spike), and action-to-endpoint correlation (layer 5). Also deferred: sharing one robots cache / politeness gate between the resolver and discovery (today each builds its own, so a run fetches `robots.txt` twice) — a bounded, known optimization, not a correctness gap.

**Decision — §2b layer-1 second half (spec references in the landing-page HTML).** When conventional-path probing finds nothing, the run now fetches the landing page and scans its HTML for a *reference* to a spec, then probes each candidate. References are found generically (§0): Redoc's `spec-url` attribute, and any quoted, whitespace-free token carrying the spec vocabulary (`openapi`/`swagger`/`api-docs`) — which covers a Swagger-UI `url:` config, a `<link>`/`<a href>`, or an inline path. Candidates are resolved against the page URL, de-duplicated, and **capped** (`_MAX_HTML_SPEC_CANDIDATES`) to stay bounded and near-free. The reference-finding is deliberately *loose* because the **validation is strict**: every candidate goes through the same JSON `openapi`/`swagger`-version-key check as a conventional probe, so a false lead (a `swagger-ui.html` shell, an unrelated `.json`) costs one bounded fetch and is rejected, never reported — probes must not produce false positives (same guarantee as the first half). Same non-negotiables carry over: `robots.txt` allow/deny is honored on both the landing fetch and each candidate (§6), the probes are crawl-delay-exempt, every failure is swallowed into "not there", and a hit is reported as *observed*, never a complete-API claim. Deliberately scoped, deferred (not dropped): **scanning referenced JS bundles** (this slice reads only the landing HTML — bundle scanning adds a fetch per `<script src>` and wants its own bound); **YAML specs** (validation stays JSON-only, matching the conventional paths — a YAML-referenced spec is found but fails the JSON check for now). Known cost, accepted for this slice: on a conventional-path miss the run fetches the landing page again (the resolver already fetched it), the same bounded double-fetch already noted for `robots.txt` above — reusing the resolver's HTML is the deferred optimization.
